...
def Solve(days, doses, params, x, t, initial_condition, println, step = 1):
    ...
    for it in range(len(t)):
        ...
        if it in days:
            index = days.index(it)
            A, B = Matrix(doses[index], True, params, X[1], T[1], len(x))

        else: 
            A, B = Matrix(0., False, params, X[1], T[1], len(x))

        S[it] = X_K
        E = B @ X_K
        X_K = np.linalg.solve(A, E)
        if it % step == 0 and println:
            print(f'Iteração: {it}/{len(t)}')
